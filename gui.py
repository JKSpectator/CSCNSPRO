...
class ApplicationWindow(QWidget):
	def __init__(self):
		...
		self.simulation = QGridLayout(self)
		self.makeControls()
		self.topGrid.addLayout(self.controls, 1, 0)
		self.topGrid.addLayout(self.simulation, 2, 0)
	...
	def makeInput(self):
		...
		self.incLabel = QLabel("轨道平面倾斜度 (deg):")
	...
	def makeControls(self):
		...
		self.simulation.setSpacing(5)  # space between buttons

		# make spawn simulation button
		self.generateButton = QPushButton("生成")
		self.generateButton.setToolTip("创建新的模拟进程")
		...
		self.killButton = QPushButton("关闭")
		self.killButton.setToolTip("杀死最后创建的模拟进程")
		...
		self.togglePlayButton.setToolTip("暂停/启动模拟进程")
		...
		self.controls.addWidget(self.pathNodeNumLabel, 14, 0)
		self.controls.addWidget(self.totalNumLinksLabel, 15, 0)

		# 测时延按钮
		self.testButton = QPushButton("test")
		self.testButton.setToolTip('test delay')
		self.controls.addWidget(self.testButton, 19, 0)
		self.testButton.clicked.connect(self.testDelay)

		self.delay_idealTimeLabel = QLabel('---')
		self.delay_noTimeLabel = QLabel('---')
		self.delay_normalTimeLabel = QLabel('---')

		self.simulation.addWidget(self.delay_idealTimeLabel, 23, 0)
		self.simulation.addWidget(self.delay_noTimeLabel, 24, 0)
		self.simulation.addWidget(self.delay_normalTimeLabel, 25, 0)
		...
		while True:
			try:
				...
				if type(recv) == str:
					...
					elif command == 'pathNodeNumber':
						temp = str(recv[1])
						self.pathNodeNumLabel.setText('Path node number: ' + temp)
	...
	def testDelay(self):
		try:
			self.myPipeConn.send('testDelay')
		except EOFError:
